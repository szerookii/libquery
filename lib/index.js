...
class LibQuery {
    ...
    static async query(host, port) {
        return new Promise(async(resolve, reject) => {
            try {
                let sessionId = this.generateSessionId();
                let client = dgram.createSocket("udp4");
                let challenge = await this.generateChallenge(client, sessionId, host, port);
    ...
    static async generateChallenge(client, sessionId, host, port) {
